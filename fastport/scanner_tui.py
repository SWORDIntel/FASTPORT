...
╔══════════════════════════════════════════════════════════════════════╗
║                                                                      ║
║       HDAIS Fast Port Scanner - Interactive TUI                     ║
║       Ultra-Fast Scanning with Async & Masscan Support              ║
║       Version 2.0.0                                                  ║
║                                                                      ║
╚══════════════════════════════════════════════════════════════════════╝
            """
    ...
    def main_menu(self) -> str:
        ...
        if HAS_RICH:
            ...
            table.add_row("6", "🚀 Start Port Scan")
    ...
    def select_gpu_report(self):
        ...
        if file_path and Path(file_path).exists():
            ...
            if HAS_RICH:
                self.console.print(f"\n[green]✓[/green] Selected: {file_path}")
                input("\nPress Enter to continue...")
        elif file_path:
            if HAS_RICH:
                self.console.print(f"\n[red]✗[/red] File not found: {file_path}")
            else:
    ...
    def configure_scanner_mode(self):
        ...
        if HAS_RICH:
            self.console.print(Panel(
                ...
                "  • Requires masscan installed (apt-get install masscan)\n"
                "  • Best for scanning large port ranges\n"
            ...
            if masscan.is_available():
                self.console.print("\n[green]✓[/green] Masscan is installed and available")
            else:
                self.console.print("\n[yellow]⚠[/yellow] Masscan not found (optional)")
        ...
        if HAS_RICH:
            self.console.print(f"\n[green]✓[/green] Scanner mode set to: {mode}")
            input("\nPress Enter to continue...")
    ...
    def configure_port_range(self):
        ...
        if HAS_RICH:
            ...
            table.add_row("1", "GPU/HPC/ML focused ports", "~80", "⚡ Fast")
            table.add_row("2", "Common service ports", "~150", "⚡ Fast")
            table.add_row("3", "Top 1000 common ports", "1000", "⚡⚡ Medium")
            table.add_row("4", "All ports (full scan)", "65535", "⚡⚡⚡ Slow")
            ...
            if port_range == "custom":
                ...
                self.console.print("  • custom:1-1000          (ports 1 to 1000)")
                self.console.print("  • custom:80,443,8080     (specific ports)")
                self.console.print("  • custom:1-1000,8000-9000 (multiple ranges)")

                custom = Prompt.ask("\n[cyan]Enter custom range[/cyan]", default="1-1000")
                port_range = f"custom:{custom}"

            self.scanner_options['ports'] = port_range
            self.console.print(f"\n[green]✓[/green] Port range set to: {port_range}")
            input("\nPress Enter to continue...")
        else:
            print("\nPort Ranges:")
    ...
    def advanced_settings(self):
        ...
        if HAS_RICH:
            ...
            self.console.print("\n[green]✓[/green] Advanced settings updated")
            input("\nPress Enter to continue...")
    ...
    def start_scan(self):
        ...
        if not self.scanner_options['gpu_json']:
            if HAS_RICH:
                self.console.print("\n[red]✗[/red] Please select a GPU report file first!")
            else:
                print("\n✗ Please select a GPU report file first!")
            input("\nPress Enter to continue...")
            return
        ...
        try:
            ...
            if HAS_RICH:
                self.console.print(f"\n[green]✓[/green] Scanner initialized with {len(self.scanner.ports)} ports")
                self.console.print(f"[green]✓[/green] Mode: {self.scanner.scanner_mode}")
    ...
    def show_results_summary(self):
        ...
        if HAS_RICH:
            ...
            if hosts_with_risks > 0:
                self.console.print("\n[bold yellow]⚠ High-Risk Hosts:[/bold yellow]")
            ...
            # Output files
            self.console.print(f"\n[green]✓[/green] JSON report: {self.scanner_options['output']}")
            markdown_file = self.scanner_options['output'].replace('.json', '_summary.md')
            self.console.print(f"[green]✓[/green] Markdown summary: {markdown_file}")
