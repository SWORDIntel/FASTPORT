...
fn main() {
    ...
    if target_arch == "x86_64" {
        ...
        writeln!(f, "    {{").unwrap();
        writeln!(f, "        // Check for AVX2 at runtime (stable API)").unwrap();
        ...
        writeln!(f, "        ").unwrap();
        writeln!(f, "        // VNNI detection is compile-time only on stable Rust").unwrap();
        writeln!(f, "        let has_vnni_compiled = cfg!(target_feature = \"avxvnni\");").unwrap();
        writeln!(f, "        ").unwrap();
        writeln!(f, "        if has_avx2 && has_fma && has_vnni_compiled {{").unwrap();
        writeln!(f, "            eprintln!(\"[SIMD] AVX2 + VNNI detected (Meteor Lake optimized)\");").unwrap();
        writeln!(f, "            return SIMDVariant::Avx2Vnni;").unwrap();
        ...
        writeln!(f, "    Avx2Vnni,  // Meteor Lake optimized path").unwrap();
        ...
        writeln!(f, "            SIMDVariant::Avx2Vnni => \"AVX2+VNNI (Meteor Lake)\",").unwrap();
        ...
        writeln!(f, "        matches!(self, SIMDVariant::Avx2Vnni)").unwrap();
        writeln!(f, "    }}").unwrap();
        writeln!(f, "}}").unwrap();
        writeln!(f, "").unwrap();
